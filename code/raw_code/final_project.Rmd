...
atus.sub <- subset(atus.all, atus.all$TRCHILDNUM >=1)
reg_model <- lm(CHILDCARE ~ factor(PEMARITL) + TEAGE + factor(TESEX) + HEFAMINC + TRCHILDNUM + factor(TRDPFTPT), data = atus.sub )
summary(reg_model)
```
...
```{r activity-cats}
atus.wide <- atus.all %>%
    mutate(act01 =rowSums(atus.all[,grep("t01",colnames(atus.all))]),
act02 = rowSums(atus.all[,grep("t02", colnames(atus.all))]),
           act03 = rowSums(atus.all[,grep("t03", colnames(atus.all))]),
           act04 = rowSums(atus.all[,grep("t04", colnames(atus.all))]),
           act05 = rowSums(atus.all[,grep("t05", colnames(atus.all))]),
           act06 = rowSums(atus.all[,grep("t06", colnames(atus.all))]),
           act07 = rowSums(atus.all[,grep("t07", colnames(atus.all))]),
           act08 = rowSums(atus.all[,grep("t08", colnames(atus.all))]),
           act09 = rowSums(atus.all[,grep("t09", colnames(atus.all))]),
           act10 = rowSums(atus.all[,grep("t10", colnames(atus.all))]),
           act11 = rowSums(atus.all[,grep("t11", colnames(atus.all))]),
           act12 = rowSums(atus.all[,grep("t12", colnames(atus.all))]),
           act13 = rowSums(atus.all[,grep("t13", colnames(atus.all))]),
           act14 = rowSums(atus.all[,grep("t14", colnames(atus.all))]),
           act15 = rowSums(atus.all[,grep("t15", colnames(atus.all))]),
           act16 = rowSums(atus.all[,grep("t16", colnames(atus.all))]),
           # act17 = , there is no category 17 in the data
           act18 = rowSums(atus.all[,grep("t18", colnames(atus.all))])) %>% 
...
atus.long %>%
  ...
  ggplot(aes(x=TEAGE, y=AVGMINS)) + geom_point()
```
...
atus.long %>%
  ...
  mutate(Totalmin = sum(AVGMINS_WRK)) %>%
  mutate(percent = AVGMINS_WRK/Totalmin) %>%
  ggplot(aes(x = ACTIVITY,y = percent, fill = HEFAMINC)) +
...
```{r save-plot}
 ## save the plot above
ggsave(filename = "activity-income.png", plot = last_plot(), path = "/cloud/project/final_project/figures/explanatory_figures")
```